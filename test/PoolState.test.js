...
describe("Pool State: ", function () {
    ...
    it("swap() - reverts when cp balance is not high enough for swap", async function () {
        await addBalancerLiquidity(avax_dai_pool, alice, BigNumber.from("10000000"), {})
        await addBalancerLiquidity(eth_dai_pool, alice, BigNumber.from("10000000"), {})
    ...
    it("swap() - using loop back mock, revert on sgReceive when paused", async function () {
        ...
        await addBalancerLiquidity(eth_dai_pool, alice, BigNumber.from("1000000"))
        await addBalancerLiquidity(avax_dai_pool, alice, BigNumber.from("1000000"))
    ...
    describe("LP pools are filled and fees set:", async function () {
        ...
        beforeEach(async function () {
            ...
            await addBalancerLiquidity(avax_dai_pool, alice, amount)
            await addBalancerLiquidity(eth_dai_pool, alice, amount)
            await addBalancerLiquidity(avax_dai_pool, bob, amount)
            await addBalancerLiquidity(avax_dai_pool, bob, amount)
            await equalize(endpoints, bob, false)
        ...
        it("delta() - run a series of tests NOT in fullMode and ensure audit still works", async function () {
            ...
            await addBalancerLiquidity(avax_dai_pool, alice, bigAmount)
            await addBalancerLiquidity(eth_dai_pool, alice, bigAmount)
            await addBalancerLiquidity(avax_dai_pool, bob, bigAmount)
            await addBalancerLiquidity(eth_dai_pool, bob, bigAmount)
            await addBalancerLiquidity(avax_usdc_pool, bob, bigAmount)
            await addBalancerLiquidity(eth_usdc_pool, bob, bigAmount)
            await equalize(endpoints, bob, false)
        ...
        it("addBalancerLiquidity() - reverts when the safeTransferFrom fails", async function () {
            // pause transfers
            await avax_dai_pool.token.pauseTransfers(true)
            await expect(avax_dai_pool.router.addBalancerLiquidity(avax_dai_pool.id, 1, bob.address)).to.revertedWith("Stargate: TRANSFER_FROM_FAILED")
        })
        ...
        it("instantRedeemLocal() - reverts when from address is 0x0", async function () {
            await addBalancerLiquidity(avax_dai_pool, alice, BigNumber.from(100))
            await expect(
    ...
    it("redeemLocal() - reverts when lp isnt enough", async function () {
        ...
        await addBalancerLiquidity(avax_dai_pool, alice, BigNumber.from("1"))
